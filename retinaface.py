...
class Retinaface(object):
    _defaults = {
        ...
        #   arcface训练完的权值路径
        #----------------------------------------------------------------------#
        "arcface_model_path"    : 'model_data/arcface_mobilefacenet.pth',
        #----------------------------------------------------------------------#
        #   arcface所使用的主干网络， mobilenet和inception_resnetv1
        #----------------------------------------------------------------------#
        "arcface_backbone"      : "mobilefacenet",
        #----------------------------------------------------------------------#
        #   arcface所使用到的输入图片大小
        #----------------------------------------------------------------------#
        "arcface_input_shape"   : [160, 160, 3],
        #----------------------------------------------------------------------#
        #   arcface所使用的人脸距离门限
        #----------------------------------------------------------------------#
        "arcface_threhold"      : 0.9,
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        try:
            self.known_face_encodings = np.load("model_data/{backbone}_face_encoding.npy".format(backbone=self.arcface_backbone))
            self.known_face_names     = np.load("model_data/{backbone}_names.npy".format(backbone=self.arcface_backbone))
        except:
            if not encoding:
    ...
    def generate(self):
        ...
        self.arcface    = arcface(backbone=self.arcface_backbone, mode="predict").eval()
        ...
        state_dict = torch.load(self.arcface_model_path, map_location=device)
        self.arcface.load_state_dict(state_dict, strict=False)
    ...
    def encode_face_dataset(self, image_paths, names):
        ...
        for index, path in enumerate(tqdm(image_paths)):
            ...
            crop_img = np.array(letterbox_image(np.uint8(crop_img),(self.arcface_input_shape[1],self.arcface_input_shape[0])))/255
        ...
        np.save("model_data/{backbone}_face_encoding.npy".format(backbone=self.arcface_backbone),face_encodings)
        np.save("model_data/{backbone}_names.npy".format(backbone=self.arcface_backbone),names)
    ...
    def detect_image(self, image):
        ...
        for face_encoding in face_encodings:
            ...
            #-----------------------------------------------------#
            matches, face_distances = compare_faces(self.known_face_encodings, face_encoding, tolerance = self.arcface_threhold)
            name = "Unknown"
            #-----------------------------------------------------#
            #   取出这个最近人脸的评分
